...
from cnblogs_client import CnblogsClient
...
def job(subscription_manager, github_client:GitHubClient,cnblogs_client:CnblogsClient, report_generator:ReportGenerator, notifier:Notifier, days):
    LOG.info("[开始执行定时任务]")
    ...
    # 获取爬虫数据
    content = cnblogs_client.export_progress_crawler()
        # 从Markdown文件自动生成进展简报
    report, report_file_path = report_generator.generate_daily_report_byContent(content)
    notifier.notify("博客园", report)
    LOG.info(f"[定时任务执行完毕]")
...
def main():
    ...
    # 启动时立即执行（如不需要可注释）
    job(subscription_manager, None,cnblogs_client, report_generator, notifier, config.freq_days)

    # 安排每天的定时任务
    schedule.every(config.freq_days).days.at(
        config.exec_time
    ).do(job, subscription_manager,None, cnblogs_client, report_generator, notifier, config.freq_days)
